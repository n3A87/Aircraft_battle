...
HERO_POWER_OFF_EVENT = pygame.USEREVENT + 1 #定义英雄⽆敌事件
...
class Enemy(Plane):
    def __init__(self, *groups: object) -> object:
        super().__init__(['enemy1.png'],random.randint(1,3),1000,'enemy1_down.wav',[f"enemy1_down{i}.png" for i in range(1,5)],*groups)
        self.rect.bottom = random.randint(0, screen_rect.h - self.rect.h) - screen_rect.h # 保证敌机在游戏界面内，不会超出屏幕底部
...
class Hero(Plane):
    def __init__(self, *groups):
        ...
        pygame.time.set_timer(HERO_FIRE_EVENT, 200)  # 英雄⻜机诞⽣，就开始初始化⼦弹发射
    ...
    def reset_plane(self):
        ...
        # 发布定时事件 发布英雄⽆敌时间的过时事件
        pygame.time.set_timer(HERO_POWER_OFF_EVENT, 3000)

    def fire(self,display_group):
        groups=(self.bullets_group,display_group)
        for i in range(3):
            bullet = Bullet(*groups)
            bullet.rect.centerx = self.rect.centerx
            bullet.rect.bottom = self.rect.top - i * 15
            self.mp.play_sound("bullet.wav")



class Bullet(GameSprite):
    def __init__(self,*groups):
        super(Bullet, self).__init__('bullet1.png',-12,*groups)
